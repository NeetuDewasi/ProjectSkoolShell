...
export default {
	...
	methods: {
		...
		async showData() {
			...
			formData.append("content_type", "other_downloads");
			...
			this.contents.forEach((element) => {
				this.tableData.push({
					...
					data: {
                        ...
                        item7:  element.upload_documents != null ? {
                            type: 'action',
                            options: {
                                style: 'border:none',
		...
		// 	this.$router.push(
		// 		`/school/download_center/other_view/${item.upload_content_id}`
		// 	);
		// },
