    ...
    methods: {
        ...
        async getSession() {
         ...
         async getTemplate() {
            const res = await this.callApi(
                "get",
                `/api/school/examinations/print_admit_card/templates`,
                null
            );
            ...
            if (res.status == 200) {
                var data = res.data;
                if (data.status == "success") {
                    this.template_names = data.template_list;
                    this.$refs.template_names.updateList(data.template_list);
                }
            }
        },
        selectTemplate(admitCard_id)
        {
            this.admitCard_id = admitCard_id;
        ...
        async getData(sectionId, classId) {
            ...
            const res = await this.callApi(
                "get",
                `/api/school/students/student_details/${this.sectionId}/${this.classId}`,
                null
            );
            this.log('DAATAA',res);
            if (res.status == 200) {
            ...
            this.studentList.forEach((element) => {
                this.tableData.push({
                    ...
                    data: {
                        ...
                        item2: `${element.student_first_name} ${element.student_middle_name != null ? element.student_middle_name : ''} ${element.student_last_name != null ? element.student_last_name : ''}`,
        ...
        generateAdmitCard(item,index){
            this.$router.push(`/school/examinations/generate_admit_card/${item.student_admission_id}/${this.admitCard_id}`);
        }
    },
};
</script>
